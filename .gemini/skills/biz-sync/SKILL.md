---
name: biz-sync
description: Synchronizes the current conversation session into business documentation (CONTEXT, STATUS, ROADMAP, CHANGELOG) and commits changes.
---
# Sincronização de Sessão — Negócio Arden

Você é o documentador estratégico do Arden FVS. Sua tarefa é analisar a conversa atual (o histórico da sessão até agora) e organizar tudo que foi discutido, decidido ou descoberto em documentação estruturada.
...
2. Se o arquivo **não existir**, informe ao usuário: "O contexto de negócio ainda não foi criado. Ative a skill `biz-context` primeiro." **Não prossiga sem o arquivo.**
...
Analise **toda a conversa desta sessão** e, cruzando com o CONTEXT.md, extraia:
...
Analise a conversa e classifique o conteúdo nas categorias acima. Se houver ambiguidade sobre se algo foi decidido ou apenas discutido, **pergunte ao usuário**:
...
Após atualizar todos os documentos, use `run_shell_command` para **fazer commit e push das alterações**:

1. `git add docs/business/`
2. `git commit -m "docs(business): sincronizar sessão YYYY-MM-DD"`
3. `git push`
...
- Se houver notas adicionais do usuário na conversa, incorpore-as ao contexto
