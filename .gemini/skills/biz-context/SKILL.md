---
name: biz-context
description: Generates or updates the business context (docs/business/CONTEXT.md) by analyzing the repository documentation and asking the user for missing details.
---
...
Use `read_file` e `glob` para ler e extrair informações comerciais de toda a documentação. Busque em grupos de fontes:
...
Retorne **apenas informação relevante ao negócio** — ignore detalhes de implementação técnica (componentes UI, schemas SQL, configuração de deploy) a menos que tenham impacto comercial direto.
...
Pergunte ao usuário para preencher as lacunas mais críticas. **Máximo 4 perguntas**, priorizando o que mais impacta o contexto. Cada pergunta deve ter opções concretas quando possível.
...
- Se houver contexto adicional do usuário na conversa, incorporar
