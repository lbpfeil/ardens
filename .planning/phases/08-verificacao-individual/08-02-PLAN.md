...
type: execute
wave: 2
depends_on: ["08-01"]  # Expected tight coupling: this plan extends 5 files created by 08-01 (same phase, wave 2 builds on wave 1 output). Also extends Phase 7 Server Actions and validations with new functions.
files_modified:
  ...
  - arden/lib/supabase/actions/verificacoes.ts    # Phase 7 file — extended with atualizarDescricaoVerificacao action
  - arden/lib/validations/verificacao.ts           # Phase 7 file — extended with atualizarDescricaoSchema
autonomous: false
...
- `handleExcecao`:
  ...
  2. **Item-level bulk update:** After marking the verificação as exceção, also bulk-update ALL items to status 'excecao'. This can be done in two ways:
     - **Option A (preferred):** Extend `atualizarResultadoVerificacao` Server Action to also run `UPDATE itens_verificacao SET status = 'excecao' WHERE verificacao_id = ? AND status != 'excecao'` when resultado is 'excecao'. This keeps it atomic in one server round-trip.
     - **Option B (fallback):** After the verificação update succeeds, call a separate bulk update via Supabase client in the Server Action.
     Either way, items must be marked as 'excecao' so that the checklist UI reflects the exceção state (disabled toggles, excecao left-border color on all items).
  3. On success: toast, close modal, optimistically update local `itens` state to set all items to status 'excecao': `setItens(prev => prev.map(i => ({ ...i, status: 'excecao' as const })))`. The Server Action + revalidatePath will also update the page data on next navigation.
  4. On error: toast.error
- Pass `onExcecaoClick={() => setExcecaoModalOpen(true)}` to header
- Render ExcecaoModal
- Determine `isExcecao` from verificacao status/items: `const isExcecao = verificacao.resultado === 'excecao' || itens.every(i => i.status === 'excecao')`
  </action>
  <verify>
