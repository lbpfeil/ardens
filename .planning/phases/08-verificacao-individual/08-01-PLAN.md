...
files_modified:
  ...
  - database/schema.sql
  - arden/lib/supabase/queries/verificacoes.ts   # Phase 7 file — extended to include descricao in select and VerificacaoComItens interface
autonomous: true
...
must_haves:
  truths:
    ...
    - "Verificação result auto-computes CLIENT-SIDE via computed values (allItemsMarked, hasAnyNC) in verificacao-individual-client.tsx that drive banner display. The DB trigger atualizar_contadores_verificacao handles counter updates server-side. Client does NOT rely on trigger for UI — banners are derived from local itens state."
  artifacts:
    ...
    - path: "arden/app/app/obras/[id]/verificacoes/[verificacaoId]/_components/verificacao-individual-client.tsx"
      provides: "Client orchestrator with optimistic updates and client-side result computation"
      min_lines: 80
...
**Step 1b: Verify DB trigger exists**

Before proceeding, verify that the `atualizar_contadores_verificacao` trigger exists and handles counter updates (itens_conformes, itens_nao_conformes, total_itens) correctly. Run via `mcp__supabase__execute_sql`:

```sql
SELECT trigger_name, event_manipulation, action_statement
FROM information_schema.triggers
WHERE event_object_table = 'itens_verificacao'
  AND trigger_name LIKE '%contadores%';
```

If the trigger does NOT exist, log a warning but continue — the client-side UI (Plan 08-02, Task 2) will compute result banners from local state (allItemsMarked, hasAnyNC) without depending on the trigger. The trigger is a server-side optimization for list views, not a blocker for this plan.
...
- NC Modal: render `ItemNCModal` at the bottom, controlled by `pendingNC` state:
  ...
  - `onCancel={() => setPendingNC(null)}` (rollback — see mechanism below)
  - `itemNome={pendingNC?.itemNome}`

**NC Modal Rollback Mechanism (explicit clarification):**

The ToggleGroup `value` is derived from the `itens` state array (`item.status`). When user clicks NC:
1. `onValueChange` fires with `value === 'nao_conforme'`
2. The handler sets `pendingNC` state but does NOT call `onItemMark` — therefore `itens` state is NOT updated
3. Since `itens` state is unchanged, the ToggleGroup still renders the PREVIOUS value (the old item.status)
4. The NC modal opens

On NC modal **confirm**: `onItemMark` IS called, which updates `itens` state optimistically, so ToggleGroup now shows 'nao_conforme'.

On NC modal **cancel/dismiss**: `setPendingNC(null)` clears the pending state. Since `itens` was never modified, ToggleGroup continues showing the previous value. No explicit rollback logic is needed — the old value was never overwritten.
