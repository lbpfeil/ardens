...
2. Implement `getDashboardKPIs(obraId: string): Promise<DashboardKPIs>`:
   ...
   **CRITICAL: Join path for RLS filtering:**
   - itens_verificacao -> verificacoes (via verificacao_id)
   - verificacoes -> obras (via obra_id)
   - Filter by verificacoes.obra_id = obraId

   RLS will automatically filter based on user's obra access via usuario_obras table.

   **Metric formulas (CORRECT enum usage):**

   The `itens_verificacao` table has TWO status columns with DIFFERENT enums:
   - `status` uses `status_inspecao` enum: ('nao_verificado', 'conforme', 'nao_conforme', 'excecao')
   - `status_reinspecao` uses `status_reinspecao` enum: ('conforme_apos_reinspecao', 'retrabalho', 'aprovado_com_concessao', 'reprovado_apos_retrabalho')

   **Taxa de Conformidade** = Conformes / Total verificados * 100
   WHERE Conformes = items where:
     - `status = 'conforme'` (first inspection OK)
     - OR `status_reinspecao IN ('conforme_apos_reinspecao', 'aprovado_com_concessao')` (resolved after reinspection)

   **IRS (Indice de Retrabalho)** = Retrabalhos / Total verificados * 100
   WHERE Retrabalhos = items where:
   ...
   **Concluidas** = Count of verified items (not pending)
   WHERE:
...
```
  ...
  <done>File exists with getDashboardKPIs function that returns DashboardKPIs type with correct enum references</done>
</task>
