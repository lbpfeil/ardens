...
must_haves:
  truths:
    ...
    - "Clicking NC shows hover visual feedback (modal deferred to future phase)"
...
4. Render NC list:
   ...
   **NOTE: Click behavior is hover feedback only for now (modal deferred to future phase).**
   The `cursor-pointer` and `hover:bg-surface-200` provide visual feedback, but no onClick handler needed yet.
...
- Hover feedback on NC items (cursor-pointer, bg change)
- "Ver todas" link present (non-functional placeholder is acceptable)
- NC detail modal is NOT implemented (deferred to future phase)
</success_criteria>
