...
Verificar que todos os schemas exportam corretamente via: `grep -c "export const" arden/lib/validations/verificacao.ts` deve retornar 5.
  ...
  <done>5 schemas Zod e 1 type ActionResult exportados, compilando sem erros TypeScript.</done>
</task>
...
Verificar que importa de `@/lib/supabase/server` (NAO client).
  ...
  <done>3 Server Actions exportadas (criarVerificacao, atualizarResultadoVerificacao, atualizarStatusVerificacao), todas com validacao Zod, retorno ActionResult, e checagem de imutabilidade.</done>
</task>
...
- Criacao de verificacao automaticamente cria itens a partir do template do servico
