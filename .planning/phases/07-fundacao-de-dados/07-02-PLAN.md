...
Importar `ActionResult` de `@/lib/validations/verificacao` (criado no Plan 01, ou definir localmente se Plan 01 nao tiver sido executado ainda -- ambos os plans estao na Wave 1).

NOTA: Se Plan 01 ainda nao foi executado, definir `ActionResult` localmente neste arquivo. Duplicacao temporaria e aceitavel -- pode ser refatorado depois.
  </action>
  <verify>
...
Verificar export: `grep "export async function bulkVerificar" arden/lib/supabase/actions/bulk-verificar.ts` retorna match.
  ...
  <done>Server Action `bulkVerificar` exportada, validando input com Zod (max 500 pares), chamando RPC `bulk_verificar`, e retornando contagem de criados/ignorados/reinspecionados.</done>
</task>
...
- Server Action wrapper com validacao Zod e retorno ActionResult
