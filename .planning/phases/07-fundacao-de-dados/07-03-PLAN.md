...
must_haves:
  truths:
    ...
    - "RLS policies otimizadas com initPlan pattern para performance em bulk"
  artifacts:
  ...
  key_links:
    ...
    - from: "arden/lib/supabase/queries/verificacoes.ts"
      ...
      via: "import createClient (browser client for reads)"
...
<task type="auto">
  ...
  <action>
Criar arquivo `arden/lib/supabase/queries/verificacoes.ts` seguindo o padrao existente dos outros arquivos em `queries/` (importam `createClient` de `@/lib/supabase/client` -- browser client para leitura).
...
**Funcao `getMatrizData(obraId: string): Promise<MatrizData>`:**
...
**Funcao `getVerificacaoComItens(verificacaoId: string)`:**

Query auxiliar para a pagina de verificacao individual (sera usada na Fase 8):
...
```typescript
export async function getVerificacaoComItens(verificacaoId: string): Promise<VerificacaoComItens | null> {
...
Verificar que importa de `@/lib/supabase/client` (browser client para reads).
  </verify>
  <done>2 funcoes de query exportadas: getMatrizData (3 fetches paralelos, lookup Map O(1)) e getVerificacaoComItens (verificacao com itens nested ordenados). Tipos MatrizData, MatrizServico, MatrizAgrupamento, MatrizVerificacao, VerificacaoComItens, ItemVerificacao exportados.</done>
</task>
...
3. RLS policies de verificacoes e itens_verificacao usam `(SELECT fn())` pattern
4. `database/rls-policies.sql` reflete as policies otimizadas
5. Query da matriz usa Promise.all para 3 fetches paralelos
6. Map de verificacoes usa chave `servico_id:unidade_id` para O(1) lookup
</verification>

<success_criteria>
- getMatrizData retorna servicos ativos, agrupamentos com unidades, e verificacoes em Map O(1)
- getVerificacaoComItens retorna verificacao completa com itens ordenados por ordem do template
